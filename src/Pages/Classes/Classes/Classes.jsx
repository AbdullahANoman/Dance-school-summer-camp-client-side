...
const Classes = () => {
  ...
  const handleSelect = (item) => {
    ...
    if (user && user?.email) {
      ...
      fetch(
        `https://summer-camp-server-tau.vercel.app/student/${user?.email}`,
        {
          method: "POST",
          headers: {
        ...
        .then((data) => {
          if (data?.insertedId) {
            ...
            Swal.fire({
              position: "top-end",
              icon: "success",
              title: "Your Class Added in the cart ",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-10 gap-10 mx-10 justify-evenly py-20">
        {filteredClasses.map((classes) => (
          <div
            className={
              classes?.seats == 0
                ? "card w-96 bg-red-400 shadow-xl shadow-red-800"
            ...
            <div className="card-body ">
              ...
              <div className="card-actions">
                <button
                  ...
                  disabled={classes?.seats == 0 || isAdmin || isInstructor}
